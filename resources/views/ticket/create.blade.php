{{-- Create Ticket --}}
<x-app-layout>

    {{-- =========================================================
        HEADER
    ========================================================== --}}

    <x-slot name="header">

        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">

            <div>

                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-100 leading-tight">
                    {{ __('ticket.create_ticket') }}
                </h2>

                <p class="mt-0.5 text-sm text-gray-500 dark:text-gray-400">
                    {{ __('ticket.create_ticket_description') }}
                </p>

            </div>


            {{-- Back Button --}}
            <a
                href="{{ route('ticket.index') }}"
                class="inline-flex items-center gap-2 self-start rounded-xl border border-gray-300 bg-white px-3.5 py-2 text-sm font-medium text-gray-700 shadow-sm transition hover:bg-gray-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-gray-700"
            >

                <svg
                    class="h-4 w-4 rtl:rotate-180"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    stroke-width="2"
                    aria-hidden="true"
                >
                    <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        d="M15 19l-7-7 7-7"
                    />
                </svg>

                {{ __('ticket.back') }}

            </a>

        </div>

    </x-slot>


    {{-- =========================================================
        PAGE
    ========================================================== --}}

    <div class="py-8">

        <div class="mx-auto max-w-2xl px-4 sm:px-6 lg:px-8">

            <div
                class="overflow-hidden rounded-2xl border border-gray-200/80 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800"
            >

                <div
                    class="df-page"
                    style="max-width:760px;"
                >


                    {{-- =================================================
                        PAGE HEADER
                    ================================================== --}}

                    <h1
                        class="df-display"
                        style="
                            font-size:26px;
                            font-weight:700;
                            margin:0 0 4px;
                        "
                    >
                        {{ __('ticket.create_ticket') }}
                    </h1>


                    <p
                        style="
                            color:var(--text-muted);
                            font-size:14px;
                            margin:0 0 28px;
                        "
                    >
                        {{ __('ticket.submit_new_support_request') }}
                    </p>


                    {{-- =================================================
                        FORM
                    ================================================== --}}

                    <form
                        method="POST"
                        action="{{ route('ticket.store') }}"
                        enctype="multipart/form-data"
                        id="createForm"
                        style="padding:26px;"
                    >

                        @csrf


                        {{-- =================================================
                            Ticket Title
                        ================================================== --}}

                        <div style="margin-bottom:22px;">

                            <label class="df-field-label">

                                {{ __('ticket.ticket_title') }}

                                <span class="df-req">
                                    *
                                </span>

                            </label>


                            <input
                                id="titleInput"
                                class="df-input"
                                type="text"
                                name="title"
                                style="width:100%;"
                                placeholder="{{ __('ticket.enter_ticket_title') }}"
                                value="{{ old('title') }}"
                            />


                            @error('title')

                            <div class="df-error-text">

                                {{ $message }}

                            </div>

                            @enderror

                        </div>


                        {{-- =================================================
                            Description
                        ================================================== --}}

                        <div style="margin-bottom:22px;">

                            <label class="df-field-label">

                                {{ __('ticket.description_required') }}

                                <span class="df-req">
                                    *
                                </span>

                            </label>


                            <textarea
                                class="df-input w-full h-[10rem]"
                                name="description"
                                id="description"
                            >{{ old('description') }}</textarea>


                            @error('description')

                            <div class="df-error-text">

                                {{ $message }}

                            </div>

                            @enderror

                        </div>


                        {{-- =================================================
                            Attachments
                        ================================================== --}}

                        <div style="margin-bottom:8px;">

                            <label class="df-field-label">

                                {{ __('ticket.attachments') }}

                            </label>


                            <div
                                class="df-dropzone"
                                id="dropzone"
                            >

                                <div
                                    style="
                                        color:var(--primary);
                                        margin-bottom:8px;
                                    "
                                >
                                    📤
                                </div>


                                <div
                                    style="
                                        font-weight:600;
                                        font-size:14px;
                                    "
                                >
                                    {{ __('ticket.click_to_browse_or_drag') }}
                                </div>


                                <div
                                    style="
                                        color:var(--text-muted);
                                        font-size:12.5px;
                                        margin-top:4px;
                                    "
                                >
                                    {{ __('ticket.allowed_file_types') }}
                                </div>


                                <input
                                    id="fileInput"
                                    type="file"
                                    multiple
                                    accept=".jpg,.jpeg,.png,.webp,.pdf,.mp4,.mp3,.mov"
                                    style="display:none;"
                                    name="images[]"
                                />

                            </div>


                            {{-- Uploaded Images --}}
                            <div
                                class="df-gallery"
                                id="gallery"
                            >
                            </div>


                            @error('images')

                            <div class="df-error-text">

                                {{ $message }}

                            </div>

                            @enderror


                            @foreach ($errors->get('images.*') as $messages)

                                @foreach ($messages as $message)

                                    <div class="df-error-text">

                                        {{ $message }}

                                    </div>

                                @endforeach

                            @endforeach

                        </div>


                        {{-- =================================================
                            Submit
                        ================================================== --}}

                        <div
                            style="
                                display:flex;
                                justify-content:flex-end;
                                margin-top:26px;
                            "
                        >

                            <button
                                class="df-btn df-btn-primary"
                                id="submitBtn"
                                type="submit"
                                style="
                                    min-width:150px;
                                    justify-content:center;
                                "
                            >
                                {{ __('ticket.submit_ticket') }}
                            </button>

                        </div>

                    </form>

                </div>

            </div>

        </div>

    </div>


    {{-- =========================================================
        JAVASCRIPT
    ========================================================== --}}

    <script>

        /*
        |--------------------------------------------------------------------------
        | Escape HTML
        |--------------------------------------------------------------------------
        */

        function escapeHtml(s) {

            return s.replace(
                /[&<>"']/g,
                c => ({
                    '&': '&amp;',
                    '<': '&lt;',
                    '>': '&gt;',
                    '"': '&quot;',
                    "'": '&#39;'
                }[c])
            );

        }


        /*
        |--------------------------------------------------------------------------
        | Format File Size
        |--------------------------------------------------------------------------
        */

        function formatBytes(b) {

            if (b < 1024) {

                return b + ' B';

            }

            if (b < 1024 * 1024) {

                return (
                    b / 1024
                ).toFixed(1) + ' KB';

            }

            return (
                b / (1024 * 1024)
            ).toFixed(2) + ' MB';

        }


        /*
        |--------------------------------------------------------------------------
        | Elements
        |--------------------------------------------------------------------------
        */

        const dropzone =
            document.getElementById('dropzone');

        const fileInput =
            document.getElementById('fileInput');

        const gallery =
            document.getElementById('gallery');


        /*
        |--------------------------------------------------------------------------
        | Add Files
        |--------------------------------------------------------------------------
        */

        function addFiles(files) {

            const dt = new DataTransfer();


            // Keep existing files
            Array
                .from(fileInput.files)
                .forEach(file => {

                    dt.items.add(file);

                });


            // Add new files
            Array
                .from(files)
                .forEach(file => {

                    dt.items.add(file);

                });


            // Update input
            fileInput.files = dt.files;


            renderGallery();

        }


        /*
        |--------------------------------------------------------------------------
        | Render Gallery
        |--------------------------------------------------------------------------
        */

        function renderGallery() {

            gallery.innerHTML =
                Array
                    .from(fileInput.files)
                    .map(file => `

                        <div class="df-gallery-card">

                            <img
                                src="${URL.createObjectURL(file)}"
                                alt="${escapeHtml(file.name)}"
                                class="df-gallery-thumb"
                            />

                            <button
                                type="button"
                                class="df-gallery-x"
                                data-id="${escapeHtml(file.name)}"
                                aria-label="{{ __('ticket.remove_image') }}"
                            >
                                ×
                            </button>

                            <div class="df-gallery-meta">

                                <div
                                    class="df-gallery-name"
                                    title="${escapeHtml(file.name)}"
                                >
                                    ${escapeHtml(file.name)}
                                </div>

                                <div class="df-gallery-size">
                                    ${formatBytes(file.size)}
                                </div>

                            </div>

                        </div>

                    `)
                    .join('');


            /*
            |--------------------------------------------------------------------------
            | Remove File
            |--------------------------------------------------------------------------
            */

            gallery
                .querySelectorAll('.df-gallery-x')
                .forEach(button => {

                    button.addEventListener('click', () => {

                        const dt =
                            new DataTransfer();

                        const id =
                            button.dataset.id;


                        Array
                            .from(fileInput.files)
                            .forEach(file => {

                                if (file.name !== id) {

                                    dt.items.add(file);

                                }

                            });


                        fileInput.files =
                            dt.files;


                        renderGallery();

                    });

                });

        }


        /*
        |--------------------------------------------------------------------------
        | Click To Browse
        |--------------------------------------------------------------------------
        */

        dropzone.addEventListener(
            'click',
            () => fileInput.click()
        );


        /*
        |--------------------------------------------------------------------------
        | Drag Over
        |--------------------------------------------------------------------------
        */

        dropzone.addEventListener(
            'dragover',
            event => {

                event.preventDefault();

                dropzone.classList.add('drag-over');

            }
        );


        /*
        |--------------------------------------------------------------------------
        | Drag Leave
        |--------------------------------------------------------------------------
        */

        dropzone.addEventListener(
            'dragleave',
            () => {

                dropzone.classList.remove(
                    'drag-over'
                );

            }
        );


        /*
        |--------------------------------------------------------------------------
        | Drop
        |--------------------------------------------------------------------------
        */

        dropzone.addEventListener(
            'drop',
            event => {

                event.preventDefault();

                dropzone.classList.remove(
                    'drag-over'
                );

                addFiles(
                    event.dataTransfer.files
                );

            }
        );


        /*
        |--------------------------------------------------------------------------
        | File Input Change
        |--------------------------------------------------------------------------
        */

        fileInput.addEventListener(
            'change',
            event => {

                addFiles(
                    event.target.files
                );

                event.target.value = '';

            }
        );


        /*
        |--------------------------------------------------------------------------
        | Initial Gallery
        |--------------------------------------------------------------------------
        */

        renderGallery();

    </script>

</x-app-layout>
